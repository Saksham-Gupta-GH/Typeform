...
// ──────────────────────────────────────────────────────────────
function QuestionPreview({ question, isWelcome, buttonColor }: { question: Question | null; isWelcome?: boolean; buttonColor?: string }) {
  if (!question) {
    return (
...
// ──────────────────────────────────────────────────────────────
function RightSidebar({ question, onChange }: {
          ...
          <div className="pt-2 border-t border-gray-100">
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className="text-sm text-gray-500 font-medium hover:text-black flex items-center mt-1 transition-colors"
              >
        ...
        {question.type === 'rating' && (
          <div className="pt-2 border-t border-gray-100">
            ...
            <select
              ...
              className="w-full border border-gray-200 rounded-md p-2 text-sm outline-none focus:border-blue-400 bg-white"
            >
...
export default function BuilderPage() {
        ...
        if (form) {
          ...
          if (form.design_settings) {
            setDesignSettings({
              ...
              textColor: form.design_settings.textColor || '#000000',
              buttonColor: form.design_settings.buttonColor || '#111827',
              fontFamily: form.design_settings.fontFamily || 'sans',
            });
          }
        }
  ...
  const handleDesignChange = (key: keyof typeof designSettings, value: string) => {
    ...
    (window as any).designTimeout = setTimeout(() => {
      if (formId) {
        updateForm(formId, { design_settings: newSettings }).catch(console.error);
      }
    }, 500);
  };
        ...
        {mainTab === 'content' && (
          ...
          <div className="flex items-center p-3 border-b border-gray-200 gap-1">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'content' ? 'bg-white shadow-sm text-gray-900 border border-gray-200' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}
            >
              <LayoutList size={14}/> Content
            </button>
            <button
              onClick={() => setActiveTab('design')}
              className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'design' ? 'bg-white shadow-sm text-gray-900 border border-gray-200' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}
            >
          ...
          {activeTab === 'design' ? (
            <div className="flex-1 overflow-y-auto p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-4">Design</h3>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Background Color</label>
                      <div className="flex items-center gap-2">
                        <input type="color" value={designSettings.bgColor} onChange={e => handleDesignChange('bgColor', e.target.value)} className="w-8 h-8 rounded cursor-pointer border-0 p-0" />
                        <span className="text-sm text-gray-500 uppercase">{designSettings.bgColor}</span>
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Text Color</label>
                      <div className="flex items-center gap-2">
                        <input type="color" value={designSettings.textColor} onChange={e => handleDesignChange('textColor', e.target.value)} className="w-8 h-8 rounded cursor-pointer border-0 p-0" />
                        <span className="text-sm text-gray-500 uppercase">{designSettings.textColor}</span>
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Button Color</label>
                      <div className="flex items-center gap-2">
                        <input type="color" value={designSettings.buttonColor} onChange={e => handleDesignChange('buttonColor', e.target.value)} className="w-8 h-8 rounded cursor-pointer border-0 p-0" />
                        <span className="text-sm text-gray-500 uppercase">{designSettings.buttonColor}</span>
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Font Family</label>
                      <select value={designSettings.fontFamily} onChange={e => handleDesignChange('fontFamily', e.target.value)} className="w-full text-sm border-gray-300 rounded-md shadow-sm p-2 outline-none">
                ...
                <div className="flex-1 overflow-y-auto p-3 flex flex-col h-full">
